...
if len(sys.argv) != 3:
    print('usage: ')
    print(sys.argv[0] + " netlify_site_id directory_to_deploy")
    sys.exit(1)
...
new_deploy_url = site_url + "deploys/"
new_deploy = {'files' : file_hashes, 'context': 'deploy-preview', 'draft': True}
new_deploy_json = json.dumps(new_deploy)
response = requests.post(url=new_deploy_url, headers=json_headers, data=new_deploy_json)
...
for required_hash in required_file_hashes:
    ...
    with open('output/'+current_file_name, 'rb') as current_file_handle:
